...
import * as api from '../services/apiService.native';
...
import { stableRoutePathKey, stableHistoricalRangeKey, stableJsonKey } from '../utils/stableKeys';
...
export const useAnimalData = (shouldFetch: boolean = false) => {
    ...
    const historicalKey = stableHistoricalRangeKey(historicalDateRange);
    const searchHistoryKey = stableJsonKey(searchHistory);
    ...
    const predictionsSyncKey = stableJsonKey(
        (predictions ?? []).map((p: AnimalPrediction) => p?.id ?? p?.scientific)
    );
    useEffect(() => {
        predictionsRef.current = Array.isArray(predictions) ? predictions : [];
    }, [predictionsSyncKey]);
    const navigationAlertRef = useRef(navigationAlert);
    const navigationAlertKey = stableJsonKey(navigationAlert);
    useEffect(() => {
        navigationAlertRef.current = navigationAlert;
    }, [navigationAlertKey]);
    ...
    const checkBackend = useCallback(async () => {
        try {
            ...
            // Increased retry attempts for initial health check during cold start
            const ok = await api.checkBackendHealth();
            if (!ok) {
                // If health check fails, try a simple wake-up ping
                const woken = await api.wakeUpBackend();
                if (woken) {
                   const secondTry = await api.checkBackendHealth();
                   if (secondTry) {
    ...
    const wildlifeFetchInFlight = useRef(false);
    const fetchWildlife = useCallback(async () => {
        if (wildlifeFetchInFlight.current) return;
        wildlifeFetchInFlight.current = true;
        // If a route is active, we IGNORE historical mode and strictly show current to 30 days
        const isHistoricalActive = historicalMode && historicalDateRange && !routePathKey;
        ...
        try {
            ...
            setRecentSightings(Array.isArray(data) ? data : []);
        } catch (err) {
            logger.error('Failed to fetch wildlife sightings', err);
            setRecentSightings([]);
        } finally {
            setIsWildlifeLoading(false);
            wildlifeFetchInFlight.current = false;
        }
    }, [historicalMode, historicalKey, routePathKey]);
    ...
    const getPredictionsForArea = useCallback(async (location: Location): Promise<AnimalPrediction[]> => {
        ...
        try {
            ...
            const sightingSets: { scientificName: string; sightings: Sighting[] }[] = (await Promise.all(animalSightingsPromises))
                .filter((set: { scientificName: string; sightings: Sighting[] }) => set && Array.isArray(set.sightings) && set.sightings.length > 0);
            ...
            if (!Array.isArray(pathPredictions) || pathPredictions.length === 0) {
                setStatus(AppState.SUCCESS);
                setMessage(`Found ${sightingSets.length} species with sightings, but AI predictions are not available on mobile.`);
                setPredictions([]);
                return [];
            }
            ...
            const detailedPredictionsPromises = pathPredictions.map(async (predGroup: { scientificName: string; predictions: { lat: number; lon: number }[] }) => {
                ...
                if (!sightingSet || !Array.isArray(sightingSet.sightings) || sightingSet.sightings.length === 0 || !Array.isArray(pathPoints) || pathPoints.length === 0) return null;

                const animalInfo = (ANIMALS as any)[scientificName];
                if (!animalInfo) return null;

                const currentSighting = sightingSet.sightings[0];
                if (!currentSighting) return null;

                const currentPoint = { lat: Number(currentSighting.lat), lon: Number(currentSighting.lon) };
                ...
                const waypoints: [number, number][] = [
                    ...
                    ...pathPoints.map((p: { lat: number; lon: number }) => [Number(p?.lat), Number(p?.lon)] as [number, number])
                ];
            ...
            const newPredictions = (await Promise.all(detailedPredictionsPromises)).filter(p => p !== null) as AnimalPrediction[];
    ...
    }, [searchHistoryKey, getPredictionsForArea]);
    ...
    const fetchWildlifeRef = useRef(fetchWildlife);
    fetchWildlifeRef.current = fetchWildlife;
    ...
    useEffect(() => {
        if (!shouldFetch) return;
        fetchWildlifeRef.current();
    }, [shouldFetch, historicalMode, historicalKey, routePathKey]);
                ...
                async (position: ExpoLocation.LocationObject) => {
                    try {
                        ...
                        if (now - lastRerouteTimestampRef.current > REROUTE_COOLDOWN_MS) {
                            ...
                            const preds = Array.isArray(predictionsRef.current) ? predictionsRef.current : [];
                            for (const animal of preds) {
                                if (!animal || !Array.isArray(animal.fullPath) || animal.fullPath.length < 2) continue;
