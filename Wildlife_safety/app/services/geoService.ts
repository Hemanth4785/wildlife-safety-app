...
export const calculateDistance = (loc1: Location | {lat: number, lon: number}, loc2: Location | {lat: number, lon: number}): number => {
    ...
    const lon1 = Number(loc1.lon);
    const lat2 = Number(loc2.lat);
    const lon2 = Number(loc2.lon);
...
export const getMidpoint = (loc1: Location, loc2: Location): Location => {
    if (!loc1 || !loc2) return { lat: 0, lon: 0, name: 'Invalid' };
    const lat1 = Number(loc1.lat);
    const lon1 = Number(loc1.lon);
    const lat2 = Number(loc2.lat);
    const lon2 = Number(loc2.lon);
    ...
    if (!Number.isFinite(lat1) || !Number.isFinite(lon1) || !Number.isFinite(lat2) || !Number.isFinite(lon2)) {
        return { lat: 0, lon: 0, name: 'Invalid' };
    }
    ...
    const lat3 = Math.atan2(Math.sin(rLat1) + Math.sin(rLat2), Math.sqrt((Math.cos(rLat1) + Bx) * (Math.cos(rLat1) + Bx) + By * By));
 ...
 */
export const getPathDataFromLocation = (userLocation: Location, routePath: [number, number][] | undefined | null): { remainingPath: [number, number][]; distanceToPathKm: number; closestPointIndex: number } => {
    if (!routePath || !Array.isArray(routePath) || routePath.length < 2) {
        return { remainingPath: [], distanceToPathKm: Infinity, closestPointIndex: 0 };
    }
