...
export const wakeUpBackend = async (): Promise<boolean> => {
    ...
    try {
        ...
        const response = await fetch(`${baseUrl}/`, { 
            ...
            // Small timeout for wake-up, we just want to send the signal
            signal: AbortSignal.timeout(10000) 
        } as any);
        return response.ok;
    } catch (e) {
        logger.warn("[API] Backend wake-up ping failed (might still be waking up)", e);
        return false;
    }
};
...
export const checkBackendHealth = async (): Promise<boolean> => {
    ...
    try {
        const controller = new AbortController();
        // Increase timeout to 60s for Render cold starts and slower network environments
        const timeoutId = setTimeout(() => {
            logger.warn(`[API] Health check timed out after 60s for ${url}`);
            controller.abort();
        }, 60000);
