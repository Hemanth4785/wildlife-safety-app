...
export const useChat = (
  ...
  const getSpeciesTips = (species: string[]) => {
    ...
    species.forEach(s => {
      ...
      Object.keys(animalSafetyTips).forEach(key => {
        if (lower.includes(key)) {
          animalSafetyTips[key].forEach(t => tips.add(t));
        }
      });
    });
        ...
        if (intent === "LOCATION_ONLY" && confidence > 0.7) {
          ...
          case "AREA_RISK": {
            ...
            if (p) {
              ...
              const safePlaces = await findSafePlacesNear(p.lat, p.lon, 10);
              const safetyAreasStr = safePlaces.slice(0, 5).map(s => s.name).join(";");
          ...
          case "IMAGE_SEARCH": {
            ...
            const animalNames =
              Object.values(ANIMALS).map(a => a.common.toLowerCase());
